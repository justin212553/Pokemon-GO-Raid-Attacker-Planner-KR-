...
export function EliteTMTracker({ allParties, tmOrders, setTmOrders }: EliteTMTrackerProps) {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-1 items-center gap-2 min-w-0">
                  <span className={`w-30 min-w-[70px] text-sm font-bold truncate ${isDisabled ? 'text-slate-400' : 'text-slate-200'}`} title={item.pokemonName}>
                    {item.pokemonName}
                  </span>
                  <span className="text-slate-600 text-xs font-black shrink-0">|</span>
                  <span className={`w-30 min-w-[70px] text-xs font-medium truncate ${isDisabled ? 'text-slate-500' : 'text-slate-400'}`} title={item.moveName}>
                    {item.moveName}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 ml-auto shrink-0">
                  ...
                  {!isDisabled && (
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${moveType === 'fast' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'bg-rose-500/20 text-rose-300 border border-rose-500/30'}`}>
                      {moveType === 'fast' ? '일반' : '스페셜'}
                    </span>
                  )}
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-16 py-8 mt-8 border-t border-slate-800/50">
